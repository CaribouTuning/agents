...
export class OverworldScreen extends Screen {
  ...
  _noticeEvolutions() {
    ...
    for (const mon of st.party) {
      ...
      // Keyed on the individual, not on the species.
      //
      // It used to be the species, and two of the same one — a Turtwig ready
      // to change and a Turtwig that is not — took it in turns: the young one
      // cleared the mark, the old one set it again, and the notice fired for
      // ever. A box you cannot get out of, from carrying a spare.
      const key = `${mon.uid || mon.species}:${mon.species}`;
